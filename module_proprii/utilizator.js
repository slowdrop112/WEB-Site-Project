...
class Utilizator{
    ...
    constructor({id, username, nume, prenume, email, parola, rol, culoare_chat="black", poza}={}) {
        ...
        for(let prop in arguments[0]){
            this[prop]=arguments[0][prop] //iau pe rand fiecare prop din argument(constructor) si setez ca acest prop sa aibe val din acel prop
        }
        if(this.rol)
            this.rol=this.rol.cod? RolFactory.creeazaRol(this.rol.cod):  RolFactory.creeazaRol(this.rol); //daca e obiect de tip rol setez rolul, daca e string creez si setez
    ...
    checkName(nume){
        return nume!="" && nume.match(new RegExp("^[A-Z][a-z]+$")) ; //prima litera mare si cel putin o litera mica din cauza +
    }
...
//xjxwhotvuuturmqm
    ...
    async trimiteMail(subiect, mesajText, mesajHtml, atasamente=[]){
        var transp= nodemailer.createTransport({ //nodemailer pentru a lucra cu gmail
            service: "gmail",
            secure: false,
            auth:{//date login 
